...
# This will output the temperature evolution to Results/test_nu.dat and Results/test_nu_Neff.dat the Neff value
#####################################################################
import os
import sys
import numpy as np
...
if SPIN == 'B' or SPIN == 'BOSE' or SPIN == 'Bose':
    def rho_DM(T,MDM):
        if T < MDM/30.0:
            ...
            return gDM/(2*np.pi**2)*T**4*quad(lambda E: E**2*(E**2-(MDM/T)**2)**0.5/(np.exp(E)-1.) ,MDM/T,100,epsabs=1e-12,epsrel = 1e-12)[0]

    def p_DM(T,MDM):
        if T < MDM/30.0:
            return 0.0
        else:
            return gDM/(6*np.pi**2)*T**4*quad(lambda E: (E**2-(MDM/T)**2)**1.5/(np.exp(E)-1.) ,MDM/T,100,epsabs=1e-12,epsrel = 1e-12)[0]

    def drho_DMdT(T,MDM):
        if T < MDM/30.0:
            return 0.0
        else:
            return gDM/(2*np.pi**2)*T**3*quad(lambda E: 0.25*E**3*(E**2-(MDM/T)**2)**0.5*np.sinh(E/2.0)**-2 ,MDM/T,100,epsabs=1e-12,epsrel = 1e-12)[0]
...
if SPIN == 'F' or SPIN == 'Fermi' or SPIN == 'Fermion':
    ...
    def drho_DMdT(T,MDM):
        if T < MDM/30.0:
            ...
            return gDM/(2*np.pi**2)*T**3*quad(lambda E: 0.25*E**3*(E**2-(MDM/T)**2)**0.5*np.cosh(E/2.0)**-2 ,MDM/T,100,epsabs=1e-12,epsrel = 1e-12)[0]
...
def rho_e(T):
    if T < me/30.0:
        ...
        return 4./(2*np.pi**2)*T**4*quad(lambda E: E**2*(E**2-(me/T)**2)**0.5/(np.exp(E)+1.) ,me/T,100,epsabs=1e-12,epsrel = 1e-12)[0]

def p_e(T):
    if T < me/30.0:
        return 0.0
    else:
        return 4./(6*np.pi**2)*T**4*quad(lambda E: (E**2-(me/T)**2)**1.5/(np.exp(E)+1.) ,me/T,100,epsabs=1e-12,epsrel = 1e-12)[0]
...
def drho_edT(T):
    if T < me/30.0:
        ...
        return 4./(2*np.pi**2)*T**3*quad(lambda E: 0.25*E**3*(E**2-(me/T)**2)**0.5*np.cosh(E/2.0)**-2 ,me/T,100,epsabs=1e-12,epsrel = 1e-12)[0]
